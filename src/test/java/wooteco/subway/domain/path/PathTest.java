...
class PathTest {
    ...
    void calculateFare(final int distance, final int expected) {
        ...
        final Path path = new Path(List.of(일번_강남역, 이번_선릉역, 삼번_잠실역), distance, 0, new Fare());
    ...
    void calculateFare(final int distance, final int maxExtraFare, final int expected) {
        ...
        final Path path = new Path(List.of(일번_강남역, 이번_선릉역, 삼번_잠실역), distance, maxExtraFare, new Fare());
        final int actual = path.calculateFare();
